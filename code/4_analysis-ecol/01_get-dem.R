...
quail23_bbox = st_bbox(quail23) |> st_as_sfc() |> st_buffer(1000) |> st_as_sf() |> st_transform(4326)
...
quail23_dem = elevatr::get_elev_raster(quail23_bbox, src="gl1", clip = "bbox")

# Convert to rast
hast20_dem = rast(hast20_dem)
borr20_dem = rast(borr20_dem)
quail23_dem = rast(quail23_dem)
...
# Make a merged DEM
merged_dem = terra::vrt(sprc(hast20_dem, borr20_dem, quail23_dem), overwrite = TRUE)
...
# Create a version disaggregated to 3 m resolution
merged_dem_disagg = disagg(merged_dem, fact = 10, method = "bilinear")

# Write
writeRaster(merged_dem_disagg, filename = file.path(DEMS_PATH, "dem_merged_disagg.tif"), overwrite = TRUE)