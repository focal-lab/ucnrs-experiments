# Purpose: Extract veg preds and topo indices for each 3 m pixel (using the DEM grid, disaggregated
# by a factor of 10 in each dimension)
...
dem = rast(file.path(DEMS_PATH, "dem_merged_disagg.tif"))
...
covers_hast = list(covers_hast20, covers_hast23, reserve = "hast")
covers_borr = list(covers_borr20, covers_borr23, reserve = "borr")
focal_covers = list(covers_hast, covers_borr)

compiled_tabular = list()

for(i in 1:length(focal_covers)) {

  focal_20 = focal_covers[[i]][[1]]
  focal_23 = focal_covers[[i]][[2]]
  reserve = focal_covers[[i]][[3]]

  #  Make a layer for whether there was any predicted cover at all in the pixel
  contains_preds_20 = sum(focal_20, na.rm = TRUE) > 0
  contains_preds_23 = sum(focal_23, na.rm = TRUE) > 0
  ...
  indices = c(burned)
  names(indices) = c("burned")


  indices_foc = crop(indices, covers_merged)
  ...
  stack = c(covers_merged, indices_foc)

  # Check that it makes sense
  writeRaster(stack, file.path(ENV_PREDS_PATH, "TEMP_veg_preds_and_topo_indices_unmerged.tif"), overwrite = TRUE, datatype = "FLT8S")
  ...
  writeRaster(index_rast, file.path(ENV_PREDS_PATH, "preds_index_disagg.tif"), overwrite = TRUE)
  ...
  d4 = d3 |>
    ...
    select(x, y, starts_with("max_cover_type_agg"), starts_with("max_cover_area_agg"), starts_with("tot_cover_agg"),
          burned, cell_index) |>
    mutate(reserve = reserve)

  compiled_tabular[[i]] = d4
...
tabular = bind_rows(compiled_tabular)


write_csv(tabular, file.path(COMPILED_FOR_ANALYSIS_PATH, "veg_preds_and_topo_indices_highres.csv"))
