...
## Download step
```
conda create -n download python=3.10
...
# Data
All the data required to reproduce the ML experiments and mapping go into the `data` folder of this repository, broken up into `inputs`, `intermediate`, and `outputs`. The data for the ecological change analyses go into the `data-ecol` folder. These two data folders are ignored by git, so the files will not come in the repo when you clone it. However, all of the data files can be downloaded from this [box folder](https://ucdavis.box.com/v/UCNRS-UAV-vegetation-mapping) and then placed into the two data folders within this repository.

# Processing steps
The processing steps are in `code` and are broken up into four folders of scripts, corresponding to a type of operations: `1a_spatial_registration`,  `1b_semanatic_segmentation`, `2_geospatialize_imagery_predictions`, `3_analysis`, and `4_analysis-ecol`. The first two folders can be run in an arbitrary order since they do not depend on each other, but otherwise each folder must be run sequentially. Within each folder there are numbered scripts that must also be run sequentially.

## Spatial registration (folder `1a_spatial_registration`)
...
## Semantic segmentation (folder `1b_semanatic_segmentation`)
...
## Geospatialize imagery predictions (folder `2_geospatialize_imagery_predictions`)
...
## Ecologcial change analysis (folder `4_analysis-ecol`)
This section uses the final maps procued by the previous steps and performs an assessment of vegetation cover change over time. The steps include downloading a digital elevation model, using it to derive potential explanatory variables including slope and solar radiation index, assigning mapped vegetation pixels as burned or unburned based on fire perimeters, converting the spatial raster data to tabular form  for analysis, computing and visualizing transitions over time, using random forest modeling to explain whether "dead" trees ultimately resprout, and visualizing the geospatial vegetation predictions that comprised the change analysis with annotations including fire perimeter and highlighted pixels of interest.
